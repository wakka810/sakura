use sakura_core::image::{decode_cbg, decode_raw_bitmap, read_cbg_metadata};
...
fn main() {
    ...
    let mut dsc_ok=0u32; let mut dsc_err=0u32;
    let mut count=0u32;
    let paths: Vec<_> = glob_arcs();
    for f in &paths {
        let d = std::fs::read(f).unwrap();
        if &d[..12] != b"BURIKO ARC20" { continue; }
        ...
        for i in 0..cnt {
            ...
            if base+off+sz > d.len() { continue; }
            let payload=&d[base+off..base+off+sz];
            if payload.len()>=15 && &payload[..15]==b"CompressedBG___" {
                count+=1;
                let meta = match read_cbg_metadata(payload){Ok(m)=>m,Err(_)=>continue};
                ...
                match decode_cbg(payload){
                    Ok(img)=>{ ent.0+=1;
                        ...
                        if img.pixels.len()!=exp { errors.push(format!("{}#{}: pixel len {} != {} (v{} {}bpp)",f,i,img.pixels.len(),exp,meta.version,meta.bits_per_pixel)); }
                    }
                    Err(err)=>{ ent.1+=1; if errors.len()<40 { errors.push(format!("{}#{} v{} {}bpp {}x{}: {:?}",f,i,meta.version,meta.bits_per_pixel,meta.width,meta.height,err)); } }
                }
            } else if payload.len()>=15 && &payload[..15]==b"DSC FORMAT 1.00" {
                match decompress_dsc(payload){Ok(_)=>dsc_ok+=1,Err(err)=>{dsc_err+=1; if errors.len()<40 {errors.push(format!("DSC {}#{}: {:?}",f,i,err));}}}
            }
        }
    }
    println!("CBG decode results by (version,bpp) -> (ok,err):");
    for (k,v) in &ver_ok { println!("  v{} {}bpp: ok={} err={}",k.0,k.1,v.0,v.1); }
    println!("DSC ok={} err={}",dsc_ok,dsc_err);
    println!("total cbg scanned {}",count);
    println!("--- sample errors ({}) ---",errors.len());
    for e in errors.iter().take(40) { println!("{}",e); }
}
...
fn glob_arcs()->Vec<String>{
    ...
    for entry in std::fs::read_dir("サクラノ詩").unwrap(){
        ...
        if p.extension().map(|e|e=="arc").unwrap_or(false){ v.push(p.to_string_lossy().to_string()); }
    }
    v.sort(); v
}
